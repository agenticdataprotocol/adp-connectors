...
describe("HypervisorClient", () => {
  ...
  function spawnMock() {
    transport = new StdioTransport("python3", [MOCK_SCRIPT], undefined, undefined, testLogger.logger);
    transport.start();
    client = new HypervisorClient(transport, testLogger.logger);
  }
  ...
  describe("ADP protocol methods", () => {
    ...
    it("should discover resources", async () => {
      ...
      expect(result.resources[0].resourceId).toBe("demo:notes");
    ...
    it("should describe a resource", async () => {
      ...
      expect(result.usageContract.fields[0].fieldId).toBe("path");
    });
  ...
  describe("error handling", () => {
    ...
    it("should throw when sending without connection", async () => {
      const disconnectedTransport = new StdioTransport("python3", [MOCK_SCRIPT], undefined, undefined, testLogger.logger);
      const freshClient = new HypervisorClient(disconnectedTransport, testLogger.logger);
      await expect(freshClient.initialize()).rejects.toThrow("not connected");
    });

    it("should timeout requests when Hypervisor does not respond", async () => {
      const shortTransport = new StdioTransport("python3", [MOCK_SCRIPT], undefined, undefined, testLogger.logger);
      shortTransport.start();
      const shortTimeoutClient = new HypervisorClient(shortTransport, testLogger.logger, 500);
      await shortTimeoutClient.initialize();
      try {
        await (shortTimeoutClient as any).send("test.hang");
        expect.fail("should have thrown");
      } catch (err) {
  ...
  describe("authorization", () => {
    ...
    it("includes password in authorization when provided", async () => {
      spawnMock();
      client.setAuthorization("admin", "secret");
      await client.initialize();
  ...
  describe("concurrent requests", () => {
    it("should handle multiple concurrent requests correctly", async () => {
      ...
      const [, r2, r3] = await Promise.all([
        client.ping(),
        client.discover(),
        client.describe("demo:notes", "QUERY"),
      ]);
