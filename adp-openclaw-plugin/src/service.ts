...
export function registerAdpService(api: OpenClawPluginApi, config: AdpPluginConfig): ServiceResult {
  ...
  api.registerService({
    ...
    start: async () => {
      ...
      let client: HypervisorClient | undefined;
      try {
        const [command, ...args] = config.command;

        const env: Record<string, string> = config.env ? { ...config.env } : {};
        env.ADP_USERNAME = config.username;

        const transport = new StdioTransport(command, args, env, undefined, api.logger);
        transport.start();
        client = new HypervisorClient(transport, api.logger);
        client.setAuthorization(config.username, config.password);
