# ADP OpenClaw Plugin — Docker Compose Quickstart

A self-contained Docker Compose stack that launches all ADP Hypervisor backend
dependencies (PostgreSQL, pgvector, MongoDB, local filesystem) and a
ready-to-use `adp-runtime` container. OpenClaw connects to the runtime
container via the plugin's `command` configuration and starts exploring
...
| OpenClaw | with `adp-openclaw-plugin` installed |
...
```bash
cd adp-openclaw-plugin/docker
docker compose up -d
```

> The first run builds the `adp-runtime` image and pulls database images.
> This typically takes about one minute.

### 2. Install the plugin

```bash
openclaw plugin install /path/to/adp-openclaw-plugin
```

### 3. Configure the plugin

Point your OpenClaw plugin at the running container — see
[OpenClaw Plugin Configuration](#openclaw-plugin-configuration) below.
...
The stack does **not** expose a network port for the Hypervisor. Instead,
OpenClaw runs `adp-hypervisor` _inside_ the already-running `adp-runtime`
container via `docker compose exec`. This means the stack must be running
**before** OpenClaw connects.
    ...
    "entries": {
      "adp-openclaw-plugin": {
        ...
        "config": {
          ...
          "command": [
            "docker", "compose",
            "-f", "/absolute/path/to/adp-openclaw-plugin/docker/compose.yaml",
            "exec", "-iT",
            "adp-runtime",
            "python", "-m", "adp_hypervisor", "--config", "/opt/adp/config"
          ],
          "username": "demo",
          "password": "demo"
        }
...
> **Replace** `/absolute/path/to/adp-openclaw-plugin/docker/compose.yaml` with
> the actual path on your machine. You can obtain it by running:
>
> ```bash
> cd adp-openclaw-plugin/docker && pwd
> ```
...
> when OpenClaw launches the command as a subprocess (stdin is a pipe, not a
> terminal).
...
The `adp-runtime` container ships with `ADP_USERNAME=demo` / `ADP_PASSWORD=demo`
...
```bash
docker compose logs adp-runtime
```

Check the output for missing dependencies or configuration errors.

### Plugin can't connect to Hypervisor
...
2. Confirm your plugin config uses `-iT`. The `-i` flag keeps stdin open
   and `-T` disables pseudo-TTY allocation — both are required when OpenClaw
   spawns the command as a subprocess. Using `-i` alone causes
   `docker compose exec` to exit with "the input device is not a TTY".

### Backend connection refused

The `adp-runtime` container waits for backends to become healthy before
...
### Upgrading `adp-hypervisor` to a newer version

When a new version of `adp-hypervisor` is released, rebuild the `adp-runtime`
image:

```bash
docker compose build --no-cache adp-runtime
docker compose up -d
```
...
```bash
# build once
docker build -t adp-openclaw-runtime:latest .

# then in compose.yaml, replace:
#   build:
#     context: .
# with:
#   image: adp-openclaw-runtime:latest
```
...
- **stdio transport** — `adp-hypervisor` communicates via stdin/stdout and does
  not expose a network port.
